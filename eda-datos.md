...
Todos los archivos de entrada se leen del directorio actual (`.`). Todos los archivos de salida (scripts, reportes, datasets limpios) se escriben en la subcarpeta `./eda/`. Crea esa carpeta al inicio del script si no existe (`os.makedirs("eda", exist_ok=True)`).
...
2. Escribe un script `eda/eda.py` (en la subcarpeta `eda/`) que:
   - Al inicio, ejecute `os.makedirs("eda", exist_ok=True)` para garantizar que la carpeta exista.
   ...
   - Genere un archivo `eda/data_limpia.csv` con el dataset limpio y las columnas renombradas en español (imputación de nulos con mediana/moda según corresponda).
   - **Al finalizar, exporte todos los resultados del análisis a `eda/reporte_eda.md`** siguiendo la estructura definida en la sección "Estructura del reporte .md" más abajo.

3. Ejecuta el script con `bash python eda/eda.py`.
...
4. Devuelve un resumen claro en **español**, aplicando **mayúsculas solo al inicio de cada frase y en nombres propios**. Incluye:
   ...
   - Nombre del archivo limpio generado: `eda/data_limpia.csv`.
   - Nombre del reporte generado: `eda/reporte_eda.md`.

5. Si el script falla, corrige los errores y vuelve a intentarlo. No modifiques manualmente los datos fuera del script.


> **⚠️ Importante:** Este agente NO genera gráficos ni visualizaciones de ningún tipo. Las visualizaciones son responsabilidad exclusiva del agente **visualizacion-datos**. El script `eda/eda.py` no debe importar `matplotlib` ni `seaborn` ni producir ninguna imagen.
...
- Archivo generado: `eda/data_limpia.csv`
...
> **Instrucción de escritura del .md:** Usa `tabulate` con `tablefmt="pipe"` o construye las tablas manualmente en formato Markdown. Escribe el reporte en `eda/reporte_eda.md` y el script en `eda/eda.py`. El análisis cualitativo de nulos y las conclusiones deben redactarse en español, con mayúsculas solo al inicio de frase.
